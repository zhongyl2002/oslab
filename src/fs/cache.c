...
static Block *cache_acquire(usize block_no) {
    ...
    // when not found and no block is going to be evicted. => 分配新的block
    if (!slot) {
...
/*
    SUMMARY:
        在`cache.h`文件中定义了`BlockCache`结构体，在`cache.c`文件中实现方法，起到分层、封装的作用
        日志中保存块的编号，将块写入磁盘实现持久化，通过cache层实现原子操作，串行写入磁盘

        实现文件系统层次中的Buffer Cache层和Logging层 
*/